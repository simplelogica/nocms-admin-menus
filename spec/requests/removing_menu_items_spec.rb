...
describe NoCms::Admin::Menus do
  ...
  context "when removing a nested menu item", js: true do

    let(:no_cms_menu) { create :no_cms_menus_menu }
    let(:no_cms_menu_item) { create :no_cms_menus_menu_item, menu: no_cms_menu }
    let(:nested_menu_item) { create :no_cms_menus_menu_item, menu: no_cms_menu, parent: no_cms_menu_item }
    ...
    before do
      ...
      within("#content_block_#{nested_menu_item.id}") do
        click_link I18n.t('no_cms.admin.menus.menu_items.form.remove')
      end
    ...
    it "should not show the menu item in the menu edit page" do
      expect(current_path).to eq no_cms_admin_menus.edit_menu_path no_cms_menu
      expect(subject).to_not have_selector "#content_block_#{nested_menu_item.id}"
    end

  end

  context "when removing a menu item with children", js: true do
    ...
    before do
      ...
      within("#content_block_#{no_cms_menu_item.id}>.b-title>.mo") do
        click_link I18n.t('no_cms.admin.menus.menu_items.form.remove')
      end
    ...
    it "should not show the nested menu item in the menu edit page" do
      expect(current_path).to eq no_cms_admin_menus.edit_menu_path no_cms_menu
      expect(subject).to_not have_selector "#content_block_#{nested_menu_item.id}"
    end
